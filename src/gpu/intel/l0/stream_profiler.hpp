...
#ifndef GPU_INTEL_L0_STREAM_PROFILER_HPP
#define GPU_INTEL_L0_STREAM_PROFILER_HPP
...
public:
    ...
    public:
        entry_t(ze_kernel_timestamp_result_t &kernel_timestamp_result,
            ...
            start_ = kernel_timestamp_result.global.kernelStart;
            end_ = kernel_timestamp_result.global.kernelEnd;
            max_ = max_timestamp_value;
    ...
    status_t get_info(profiling_data_kind_t data_kind, int *num_entries,
        ...
        int idx = 0;
        for (auto &ev : events_) {
            const l0::event_t &l0_event
                    = *utils::downcast<l0::event_t *>(ev.event.get());

            ze_kernel_timestamp_result_t kernel_timestamp_result;
            CHECK(l0::zeEventQueryKernelTimestamp(
                    l0_event[0], &kernel_timestamp_result));
...
#endif // GPU_INTEL_L0_STREAM_PROFILER_HPP
